...
This file is the **policy** for how the agent uses MCP. **Machine definitions** live under **`orgwave/mcp/`** (see below).
...
- **`orgwave/mcp/servers/*.json`** describe each server; **`python3 orgwave/scripts/build-mcp-json.py`** merges them into **`.cursor/mcp.json`**, which Cursor loads. When env tokens are visible to Cursor, those servers **can** auto-start.
...
| Path | Role |
|------|------|
| **`orgwave/mcp/README.md`** | **How to add a server** — filename, JSON shape, `_orgwave` metadata, `${env:…}` secrets. |
| **`orgwave/mcp/servers/<id>.json`** | **Source of truth** per MCP server (one file per `<id>`). |
...
| Step | Why |
|------|-----|
| **Secrets** | Set env vars named in each server’s **`_orgwave.env`** (see `orgwave/mcp/servers/*.json`). |
...
**No.** Use Cursor **Agent** auto-run settings and optional **`~/.cursor/permissions.json`** — see **`orgwave/mcp/README.md`** (`permissionsAllowlistHint` per server) and [permissions.json reference](https://cursor.com/docs/reference/permissions). Patterns use the same **`<id>`** as `servers/<id>.json` (e.g. `github:*`).

## Required (default)

| Server | Definition | Purpose |
|--------|------------|---------|
| **GitHub** | `orgwave/mcp/servers/github.json` | Repo list/filter for discovery (same class of data as `gh` when MCP is down). |

## Optional

Add **`servers/<new-id>.json`**, run **`build-mcp-json.py`**, document in playbook **`SKILL.md`** if a playbook **requires** that integration for non-discovery steps.

## Rules for playbooks

- Do **not** embed MCP server commands in playbooks; add **`orgwave/mcp/servers/<id>.json`** and merge.
- The orchestrator always allows **`gh`** / **`discovery-output.json`** as discovery fallbacks.

## Changing MCP servers

1. Add or edit **`orgwave/mcp/servers/<id>.json`** (see **`orgwave/mcp/README.md`**).
