...
- **Optional `mcp_install`:** Comma-separated MCP server ids (`mcp-servers/servers/<id>.json`). Drives blue **Add to Cursor** badges. **`mcp_install_bridge`** (GitHub Pages origin, no trailing slash) points badges at **`docs/mcp-install.html?…`** so links work from **github.com** (raw `cursor://` in README image links often fails; see **`docs/README.md`**). Keep in sync with **`MCP servers required`** in each playbook’s **`SKILL.md`**.
