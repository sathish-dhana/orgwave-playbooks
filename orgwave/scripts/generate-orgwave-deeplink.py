...
**MCP (non-blocking):** Read `orgwave/required-mcp.md` and **`mcp-servers/README.md`**. Server wiring lives in **`mcp-servers/servers/<id>.json`** (one file per server; optional **`_orgwave`** block lists env vars like `GITHUB_TOKEN`). Those files are merged into **`.cursor/mcp.json`** — use whichever MCP servers Cursor exposes when env tokens are available. **Prefer** them for discovery when tools work; if any server is down, errors, or needs approval, **do not stop the playbook** — fall back to **`gh`** and/or **`discovery-output.json`** per the playbook. Never require me to fix MCP before continuing.
