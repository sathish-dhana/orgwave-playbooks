...
DEEPLINK_PROMPT_TEMPLATE = (
    ...
    "First complete Prerequisites in playbooks/{playbook_id}/SKILL.md. "
    "For each MCP server id under MCP servers required confirm mcpServers that id exists in repo root dot cursor mcp dot json. "
    "If a required id is missing stop and tell the user to add mcp-servers/servers that id dot json per mcp-servers README run python3 orgwave/scripts/build-mcp-json.py reload Cursor enable the server. "
    "If a required server is disabled or tools are missing stop with required-mcp enabling steps and Reload Window. "
    "For GitHub PAT remind that github-mcp-launch loads zshrc when token vars are empty so export GITHUB_TOKEN or GITHUB_PERSONAL_ACCESS_TOKEN in zshrc or gh auth login per mcp-servers README. "
