...
2. **List repositories** the token can **push** to, for the scope the user names (their user account, specific org(s), or “all my repos”):
   - **Push-accurate list:** `@modelcontextprotocol/server-github` has no **`GET /user/repos`-style** tool with **`permissions.push`**. Use **`gh api user/repos --paginate`** (filter `.permissions.push == true`) or **REST + token** for that requirement. If **`gh` is not on PATH**, try **`/opt/homebrew/bin/gh`** / **`/usr/local/bin/gh`**, or **`brew install gh`** on macOS when Homebrew exists; then prefer **`gh`** for this list. Use **GitHub MCP** for later steps (files, branches, PRs) when session tools exist.
