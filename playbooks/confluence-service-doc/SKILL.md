...
2. **`CONFLUENCE_BASE_URL`** is set in **`mcp-servers/servers/confluence.json`** (`https://confluence.myntracorp.com/`). Set **`CONFLUENCE_USER_EMAIL`** and **`CONFLUENCE_API_TOKEN`** in repo-root **`.env`**, or **`~/.cursor/confluence-mcp.env`**, or **`export`** in **`~/.zshrc`** ([**mcp-servers/README.md**](../../mcp-servers/README.md) — **`confluence-mcp-launch.mjs`** fills empty keys after **`.env`**). [`.env.example`](../../.env.example); terminal one-liners are in this folder’s **[README.md](README.md)**.
